...
// Fetch all products where category is 'Kids'
$active_tag = isset($_GET['tag']) ? $_GET['tag'] : '';
$filter_tag = isset($_GET['tag']) ? $_GET['tag'] : '';

if (!empty($filter_tag)) {
    $stmt = $conn->prepare("SELECT p.* FROM products p JOIN category c ON p.category_id = c.id WHERE c.name = 'Kids' AND p.tag = ?");
    if ($stmt === false) {
        die("MySQL Prepare Error: " . $conn->error);
    }
    $stmt->bind_param("s", $filter_tag);
} else {
    $stmt = $conn->prepare("SELECT p.* FROM products p JOIN category c ON p.category_id = c.id WHERE c.name = 'Kids'");
    if ($stmt === false) {
        die("MySQL Prepare Error: " . $conn->error);
    }
}
...
<head>
    ...
    <title>Shoes House kids Collection</title>
...
<section class="categories-section">
  ...
  <a href="kids_collection.php" class="filter-btn <?= $active_tag == '' ? 'active' : '' ?>">All</a>
  <a href="kids_collection.php?tag=best-seller" class="filter-btn <?= $active_tag == 'best-seller' ? 'active' : '' ?>">Best Seller</a>
  <a href="kids_collection.php?tag=sports" class="filter-btn <?= $active_tag == 'sports' ? 'active' : '' ?>">Sport</a>
  <a href="kids_collection.php?tag=casual" class="filter-btn <?= $active_tag == 'casual' ? 'active' : '' ?>">Casual</a>
</div>


            <div class="collection-list mt-4">
                <?php
                // Tag mapping to normalize database tags to filter classes
                $tagMap = [
                    'Featured' => 'featured',
                    ...
                    'Casual' => 'casual',
                    '' => 'all', // Empty tags
                    'none' => 'all', // Handle 'none' or similar
                    null => 'all' // Handle null tags
                ];
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    echo '<p class="text-center">No products found in kids\'s Collection.</p>';
                }
                ?>
            </div>
        </div>
